...
def _build_feature_df(features: dict) -> pd.DataFrame:
    """
    Converts a single transaction feature dict to a model-ready DataFrame.
    Uses the same build_features() pipeline as training (mode='test')
    with saved merchant/card artifacts to prevent training-serving skew.
    """
...
def _emit_kafka_event(
    ...
    Only emits if fraud probability is above threshold (fraud positives).
    Runs in a background thread — never blocks the inference response.
    """
    try:
...
async def predict(request: PredictionRequest, background_tasks: BackgroundTasks):
    """
    Real-time fraud inference endpoint.
    - Validates input via PredictionRequest Pydantic schema
    - Checks Redis cache (deduplication within TTL window)
    - Runs full feature pipeline → LightGBM inference
    - Publishes fraud event to Kafka asynchronously (non-blocking)
    - Returns fraud probability, risk score, and model version
    """
