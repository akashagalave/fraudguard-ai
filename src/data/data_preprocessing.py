...
def process_dates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info("Processed datetime → created age, hour, day_of_week, is_weekend, is_night")
    return df
...
def handle_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    engineered_cols = ["age", "hour", "day_of_week", "is_weekend", "is_night"]
    for col in engineered_cols:
        if col in df.columns:
            df[col].fillna(df[col].median(), inplace=True)
    logger.info("Handled missing values using intelligent imputation.")
    return df
