...
ARTIFACTS_PATH = BASE_DIR / "models" / "fraudguard_lightgbm" / "feature_artifacts.pkl"
...
def apply_card_stats(df: pd.DataFrame, stats: Optional[Dict]):
    ...
    if not stats:
        ...
        df["card_amt_mean"]  = (
            df["cc_num"].map(stats["card_amt"]["mean"]).fillna(df["amt"])
        )
        df["card_amt_std"]   = (
            df["cc_num"].map(stats["card_amt"]["std"]).fillna(1)
        )
        df["card_dist_mean"] = (
            df["cc_num"].map(stats["card_dist"]["mean"]).fillna(df["distance_km"])
        )
        df["card_dist_std"]  = (
            df["cc_num"].map(stats["card_dist"]["std"]).fillna(1)
        )

    df["card_amt_zscore"]  = (
        (df["amt"] - df["card_amt_mean"]) / df["card_amt_std"]
    )
    df["distance_anomaly"] = (
        (df["distance_km"] - df["card_dist_mean"]) / df["card_dist_std"]
    )
...
def build_features(
    ...
    feature_version: str = "v3_no_tier2",
):

    logger.info(f"Building features | mode={mode} | version={feature_version}")
    df = df.copy()
    ...
    if mode == "train":
        ...
        logger.info(f"Feature artifacts saved → {ARTIFACTS_PATH}")
    ...
    df = df.drop(
        columns=["cc_num", "merchant", "category"],
        errors="ignore",
    )
